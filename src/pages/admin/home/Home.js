...
const Home = () => {
  ...
  const activeBiodatas = profiles.filter(
    (profile) => !profile.matured && !profile.discard
  ).length;
  const maturedBiodatas = profiles.filter((profile) => profile.matured).length;
  const discardedBiodatas = profiles.filter(
    (profile) => profile.discard
  ).length;
  const maleBiodatas = profiles.filter(
    (profile) => profile.gender === "male"
  ).length;
  const femaleBiodatas = profiles.filter(
    (profile) => profile.gender === "female"
  ).length;
  const biodatasOnInputDate = profiles.filter(
    (profile) =>
    ...
    <div className="flex flex-col justify-center items-center gap-16 mt-10 p-5">
      ...
      <div className="flex flex-col w-full sm:flex-row ">
        ...
        {show === "addMember" ? (
          ...
          navigate("/admin/all-press")
        ) : show === "allAdvertisements" ? (
          navigate("/admin/all-adv")
        ) : show === "allMembers" ? (
          <Members />
        ) : show === "addAdvertisement" ? (
          <FormAddAdvertisement />
        ) : show === "addNewPress" ? (
          navigate("/admin/press")
        ) : show === "matrimonyUsers" ? (
          navigate("/admin/matrimony-users")
        ) : show === "officeBearerMember" ? (
          navigate("/admin/office_bearer_member")
        ) : show === "slugs" ? (
          navigate("/admin/slugs")
        ) : show === "officeBearerSamaj" ? (
          navigate("/admin/office_bearer_samaj")
        ) : show === "modifyCastes" ? (
          navigate("/admin/add-caste")
        ) : show === "activeBiodatas" ? (
          navigate("/admin/matrimony_profiles")
        ) : show === "activeBiodatas" ? (
          navigate("/admin/matrimony_profiles")
        ) : show === "addPress" ? (
          navigate("/admin/press")
        ) : show === "addPressClip" ? (
          navigate("/admin/press-clip")
        ) : show === "addPressCutout" ? (
          navigate("/admin/press-cutout")
        ) : show === "contactQueries" ? (
