...
const ConsentAddBiodata = ({ redirect, message }) => {
  const navigate = useNavigate();

  const [agreed, setAgreed] = useState(null);

  return (
    <div className=" z-50 fixed flex flex-col justify-center gap-8 items-center w-screen h-screen  top-0 left-0 bg-[#323233] bg-opacity-90 overflow-x-auto overflow-y-scroll py-5 px-5 ">
      <div className="w-full  flex-col gap-8 justify-center items-center  h-screen rounded-md overflow-y-auto">
        <div className="shadow-xl fade-in bg-[#f7f3f5] rounded-md p-8  flex  w-fit flex-col justify-center items-center  overflow-y-scroll h-fit gap-7 max-w-3xl ">
