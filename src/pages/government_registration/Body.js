  ...
  return (
    <div className="w-full flex justify-center items-center p-2 flex-col">
      <div className="mt-12 w-full flex flex-col justify-center items-center  max-w-6xl mb-8 border-[#d5d8dc] border-4 ">
        <div className="border-[#d5d8dc] border-b-4 w-full p-5 flex justify-center items-center">
          <select
            value={form}
            onChange={(e) => {
              setForm(e.target.value);
            }}
            className="flex sm:w-fit w-full overflow-x-scroll justify-center rounded-md bg-[#EF4D48] px-4 py-2 text-md font-semibold leading-6 text-white shadow-sm focus:outline-none"
          >
            {registrations.map((item, index) => {
              ...
              return (
                <option
                  value={item.subTitle}
                  key={index}
                  className="bg-white font-Poppins text-[#444] rounded-lg sm:w-fit w-full overflow-x-scroll "
                >
