...
import IndividualMember from './IndividualMembers';
...
function CorporateMembers() {
    ...
    useEffect(() => {
        axios.get(`${BASE_URL}getAllCorporateMembers`)
            .then(response => {
    ...
    const approveMember = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.put(`${BASE_URL}approveCorporateMember/${id}`)
                    .then(response => {
    ...
    return (
        <>
            ...
            <div className="overflow-x-auto fade-in w-full flex items-center justify-center mb-12">
                <p className=" text-gray-900 font-bold text-2x font-Poppins text-lg ">Corporate Members Approval</p>
                ...
                <table className="w-full border-2 border-[#305D2B] max-w-7xl">
                    <thead className="w-full">
                        <tr className="bg-[#305D2B] text-white w-full">
                            <th className="p-3 text-center border-white border-r whitespace-nowrap font-bold font-Poppins">Org Name</th>
                        ...
                        {members.filter(member => !member.approved)
                            ...
                            .map(member => (
                                <tr key={member._id} className="border-b border-[#305D2B]">
                                    <td className="p-2 border-r border-[#EF4D48] text-center text-[#333] whitespace-nowrap font-bold font-Poppins">{member.orgName}</td>
