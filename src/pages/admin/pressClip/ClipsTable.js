...
const ClipsTable = ({ clips, setClips }) => {
  ...
  async function deleteQuery(id) {
    try {
      const resBody = await fetch(`${DELETE_PRESS_CLIP}${id}`, {
        method: "DELETE",
        headers: {
    ...
    <div className="overflow-x-auto fade-in w-full flex justify-center">
      <table className="w-full border-2 border-[#305D2B] max-w-7xl">
        ...
        <tbody className="w-full">
          {clips.map(({ title, url, createdAt, _id }, index) => (
            <tr
              key={_id}
